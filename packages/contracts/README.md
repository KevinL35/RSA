...
- **`ApiEnvelope<T>`**（`index.ts`）：统一响应外壳 `{ code, message, data: T }`，与 `apps/platform-api` 实际 JSON  envelope 保持一致；具体路径与 HTTP 方法以 FastAPI 路由为准。
...
- **`openapi/`** 用于存放后端导出的 OpenAPI 文档（如 `openapi.yaml`），目标能力包括：
  ...
  - 联调 **冻结协议**、做 **版本 diff**。
- 迁移建议：接口变更时 **先更新契约**（手写或 OpenAPI 二选一作为真源），再改 `apps/platform-api` 与 `apps/web` 实现；若生成类型落地，可让生成结果替换或补充 `src/`。
...
- **后续**：在 monorepo 根增加 `package.json` workspaces，将 `contracts` 发布为内部包（如 `@rsa/contracts`），由 `apps/web`、`apps/platform-api` 依赖。
...
2. 同步修改 `apps/platform-api` 路由 / Pydantic 模型与 `apps/web` 的 API 调用与展示类型（若尚未共用本包，两处类型需人工对齐）。
