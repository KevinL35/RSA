  ...
  <div>
    <section class="config-block">
      ...
      <el-table :data="displayRows" stripe class="audit-table" :empty-text="t('governance.painAuditEmpty')">
        ...
        <el-table-column :label="t('governance.painAuditColAction')" min-width="280" align="center" fixed="right">
          <template #default="{ row }">
    ...
    <section class="config-block agent-review-block">
      ...
      >
        <el-table-column :label="t('governance.painAuditCategory')" min-width="150" align="center" show-overflow-tooltip>
          <template #default="{ row }">
            {{ verticalNameById(row.vertical_id) }}
          </template>
        </el-table-column>
        <el-table-column :label="t('governance.painAuditDimension')" min-width="130" align="center" show-overflow-tooltip>
          <template #default="{ row }">
            {{ row.dimension_6way ? dimensionLabel(row.dimension_6way) : '—' }}
          </template>
        </el-table-column>
        <el-table-column :label="t('governance.painAuditColKeyword')" min-width="320" show-overflow-tooltip>
          <template #default="{ row }">
            <div class="synonym-row">
              <el-popover
              ...
              >
                ...
                <div class="kw-popover">
                  <div class="kw-popover-title">{{ t('governance.painAuditColSynonyms') }}</div>
    ...
    >
      <el-form label-position="top" class="edit-form">
        <el-form-item :label="t('governance.painAuditCategory')" required>
          <el-select
      ...
      <template #footer>
        <el-button @click="addTopicVisible = false">{{ t('governance.painAuditCancel') }}</el-button>
...
function groupKey(row: Pick<PainAuditRow, 'vertical_id' | 'dimension_6way'>): string {
  const vid = (row.vertical_id || '__none__').trim() || '__none__'
  const dim = row.dimension_6way || '__none__'
  return `${vid}::${dim}`
}

function isAgentGroupActionRow(row: PainAuditRow, $index: number): boolean {
...
async function onDeleteQueueRow(row: PainAuditRow) {
  try {
    await ElMessageBox.confirm(
      t('governance.painAuditDeleteConfirm', { keyword: row.canonical }),
      t('governance.painAuditDelete'),
      {
        type: 'warning',
...
async function onCancelAgentRecord(row: PainAuditRow) {
  try {
    await deleteDictionaryReviewQueue(row.id)
    rows.value = rows.value.filter((r) => r.id !== row.id)
    agentReviewIds.value = agentReviewIds.value.filter((id) => id !== row.id)
    ElMessage.success(t('governance.painAuditCancelledOk'))
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    ElMessage.error(`${t('governance.painAuditCancelFail')}: ${msg}`)
  }
}
...
function queueRowVerticalId(row: PainAuditRow): string {
