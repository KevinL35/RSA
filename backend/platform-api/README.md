...
在 `backend/platform-api/` 下新建 **`.env`**（勿提交 Git），可参考 **`env.example`**；按 `docs/runbooks/env-to-run.md` 填入至少 **`SUPABASE_URL`**（**Project URL**，`https://…supabase.co`，勿填 JWT）、**`SUPABASE_SERVICE_ROLE_KEY`**（**service_role**，仅后端持有）。
...
- `infra/migrations/012_review_dimension_unmatched.sql`（TB-4 扩展：六维未命中评论池）
- `infra/migrations/013_review_analysis_dimension_status.sql`（TB-4 扩展：三分类总表六维匹配状态）
- `infra/migrations/014_sentiment_temp_tables.sql`（TB-4 扩展：positive/neutral/negative 三张临时表）
...
然后将该项目的 **Project URL** 与 **service_role** 密钥填入 `backend/platform-api/.env`。

## IDE / 类型检查

在仓库根目录用 Cursor / VS Code 打开子路径时，请将 **Python 解释器** 选为 **`backend/platform-api/.venv`**（或先 `cd backend/platform-api` 再打开该文件夹），否则可能误报「找不到 `fastapi` / `pydantic_settings`」等依赖。本目录下的 `pyrightconfig.json` 供 Pylance/Pyright 指向上述 venv。

## 安装与启动

本地若要 **同时** 起 Analysis API、本服务与前端，可在仓库根目录执行：`bash scripts/dev.sh`（见 `backend/platform-api/analysis-engine/README.md`）。
...
若目录曾从 `apps/api` 重命名而来，旧的 `.venv` 里脚本的 shebang 仍指向旧路径，会出现 `bad interpreter` 或 `ModuleNotFoundError`（如 `openpyxl`）。请删除 `backend/platform-api/.venv` 后按上文重新 `python -m venv .venv` 并 `pip install -r requirements.txt`。`backend/platform-api/analysis-engine` 同理（旧路径 `apps/rsa-model-api`）。
...
- `POST /api/v1/insight-tasks/{id}/topic-discovery`：单任务异步主题挖掘；任务状态须为 `success`，立即返回 `{ ok, job }`；同一 task 已有 `pending/running` 时返回 **409**。
- `GET /api/v1/insight-tasks/{id}/topic-discovery/jobs/latest`：返回该任务最新一条 `topic_discovery_jobs` 行，用于前端轮询。
...
- `DELETE /api/v1/insight-tasks/{id}`：删除任务（`admin`/`operator`）；关联 `reviews` / `review_analysis` / `review_dimension_analysis` / `review_dimension_unmatched` 随库级 `ON DELETE CASCADE` 清理；不存在返回 404
...
**评论抓取（TB-2）环境变量**（`backend/platform-api/.env`）：
...
- **本地闭环（自建情感+词典归因）**：另起进程运行 `backend/platform-api/analysis-engine`（见该目录 `README.md`），并在本目录 **`.env`** 的 `ANALYSIS_PROVIDER_ROUTES_JSON` 中为前端默认的 `ins_builtin` 配置 `http://127.0.0.1:8089/analyze`（变量说明见上表）
...
前端开发：在 `frontend/web` 运行 `npm run dev`，Vite 会将 `/api` 代理到 `http://127.0.0.1:8000`。
...
前端 `frontend/web` 从登录所选角色写入 `localStorage`（`rsa_user_role`），`fetch` 自动带 `X-RSA-Role`。
