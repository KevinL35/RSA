#!/usr/bin/env bash
# 本地一键启动：分析服务 (8089) + API (8000) + Web (Vite)
...
if [[ ! -d apps/analysis-service/.venv ]]; then
  echo "[dev-all] 缺少 apps/analysis-service/.venv，请先执行："
  echo "  cd apps/analysis-service && python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt"
  exit 1
fi
...
echo "[dev-all] Analysis  http://127.0.0.1:${ANALYSIS_PORT}/health"
