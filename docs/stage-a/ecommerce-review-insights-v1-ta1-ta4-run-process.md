...
下一步可执行 TA-5：在 `ml/data/splits/` 下放 `train.csv` / `val.csv` / `test.csv`，使用 `ml/configs/train_roberta_splits.yaml`，在具备 GPU 的环境运行 `python ml/scripts/train_sentiment.py --config ml/configs/train_roberta_splits.yaml`（长时间训练建议 SSH + `tmux`）。
