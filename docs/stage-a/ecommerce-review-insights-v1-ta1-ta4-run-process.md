...
下一步可执行 TA-5：在 `ml/data/splits/` 下放 `train.csv` / `val.csv` / `test.csv`，使用 `ml/configs/train_roberta_splits.yaml`，运行 `python ml/scripts/train_sentiment.py --config ml/configs/train_roberta_splits.yaml`（GPU 云主机可参考 `ml/notebooks/ta5_roberta_autodl.ipynb`）。
